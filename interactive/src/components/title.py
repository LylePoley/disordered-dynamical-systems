...
from src.components.variables.dynamical_system import id_to_dynamical_system, GeneralisedLotkaVolterra
...
def render(app: Dash, class_name: str | None = None) -> html.Div:
    ...
    def update_display_equation(n_clicks: int, dynamical_system_id: str) -> str:
        ...
        return dynamical_system.latex_equation + ",", f"Currently displaying solutions to {dynamical_system.id} equations:"

    title = "Disordered dynamical systems explorer"

    explanatory_text = [
        r"""
        This is an interactive tool to explore the dynamics of disordered dynamical systems with pairwise interactions. It includes The $N \times N$ matrix $\underline{\underline{\alpha}}$ encodes the interactions between the different agents in the model. $\underline{\underline{\alpha}}$ is a random matrix whose elements are drawn identically and independently from a Gaussian distribution with mean $\frac{\mu}{N}$ and variance $\frac{\sigma^2}{N}$. The 'Re-draw noise' button re-generates these random interaction coefficients.""",
        f"""
        Currently displaying solutions to {GeneralisedLotkaVolterra.id} equations:""",
        f"{GeneralisedLotkaVolterra.latex_equation}, ",
        
        r"""
        where $y_i$ is the state of agent $i$ and $\alpha_{ij}$ is the interaction strength between agents $i$ and $j$. The plot shows all trajectories $y_i$ through time."""
    ]
